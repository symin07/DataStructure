/***********************************************************
* Author: Gunnar Egge, MIN HO KIM
* Email: eggeg@oregonstate.edu, kimminh@oregonstate.edu
...
struct Queue* listQueueCreate() 
{
	...
	newQueue->head = (struct Link*) malloc(sizeof(struct Link));
	newQueue->tail = (struct Link*) malloc(sizeof(struct Link));
...
TYPE listQueueFront(struct Queue* queue) 
{
	...
	return queue->head->next->next->value;
   /* FIXME: You will write this function */
...
TYPE listQueueRemoveFront(struct Queue* queue) 
{
	...
	TYPE value = queue->head->next->next->value;
	struct Link *new_front = queue->head->next->next->next;

	//if we remove last element in list, tell the tail to point back at sentinel
	if(new_front == NULL){
		queue->tail->next = queue->head->next;
	}

	free(queue->head->next->next);

	queue->head->next->next = new_front;
	return value;
...
TYPE listStackTop(struct Stack* stack) 
{
	assert(stack != NULL);
	assert(listQueueIsEmpty(stack->q1) == 0);
	TYPE value = listQueueFront(stack->q1);
	return value;
	/* FIXME: You will write this function */
}
