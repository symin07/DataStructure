...
{	
     /* FIXME: You will write this function */
	struct Queue* myQue = (struct Queue*)malloc(sizeof(struct Queue));
	listQueueInit(myQue);
	return myQue;
}
...
void listQueueAddBack (struct Queue* queue, TYPE value) 
{
	...
	struct Link* newSentinel = (struct Link*)malloc(sizeof(struct Link));
	//struct Link* iterator = queue->head;
	lnk->value = value;
	lnk->next = NULL;
	if(queue->head->next == NULL){
		...
		queue->tail->next->next = lnk;
		queue->tail = newSentinel;
		queue->tail->next = lnk;
	}
...
TYPE listQueueRemoveFront(struct Queue* queue) 
{
	...
	if(!listQueueIsEmpty(queue))
	{
		...
		struct Link* sentinel = (struct Link*)malloc(sizeof(struct Link));
		sentinel->next = 0;
		freed = queue->head->next;
		queue->head->next = queue->head->next->next;
		if(queue->head->next == NULL){
			queue->tail = queue->head = sentinel;
		}
		rtvalue = freed->value; 
		free(freed);
		freed = NULL;
	}else{
		printf("no element to remove. Add element. \n");
	}
	return rtvalue;
}
...
{	
	/* FIXME: You will write this function */
	assert(stack != NULL);
	return (listQueueIsEmpty(stack->q1)) ? 1 : 0; 
}
...
void listStackPush(struct Stack* stack, TYPE value) 
{
	/* FIXME: You will write this function */
	assert(stack != NULL);
// 	struct Link* w = (struct Link*)malloc(sizeof(struct Link));
//	w->value = value;
...
/*
	...
	struct Queue* tstQ = listQueueCreate();	
	listQueueAddBack(tstQ, 1);
	listQueueAddBack(tstQ, 2);
	listQueueAddBack(tstQ, 3);
	listQueueAddBack(tstQ, 4);

	assertTrue(listQueueRemoveFront(tstQ) == 1, "first 1");
	assertTrue(listQueueRemoveFront(tstQ) == 2, "first 2");
	assertTrue(listQueueRemoveFront(tstQ) == 3, "first 3");
	assertTrue(listQueueRemoveFront(tstQ) == 4, "first 4");		
